...
if (isset($_REQUEST["idutilisateur"])) {
    ...
    $rslt = $con->query("select * from utilisateur where iduser='" . $id . "'") or die(mysqli_error($con));
...
// D�claration de la variable de session
    if (empty($_SESSION['formutilisateure'])) {
        $_SESSION['formutilisateure'] = array();
    }

    if (empty($_SESSION['formutilisateure'])) {
        $_SESSION['formutilisateure'] = array();
        $_SESSION['formutilisateure'][1] = array(
...
switch (NUM_PAGE) {
    ...
    case 2:
        ...
        if (!empty($_POST)) {
            $erreurprod = FALSE;
            $log = $_SESSION['formutilisateure'][NUM_PAGE - 1]["photoe"];
            ...
            $_SESSION['formutilisateure'][NUM_PAGE - 1] = array(
            'idutilisateur' => $_GET["idutilisateur"],
			'nome' => trim(htmlspecialchars($_POST["nome"])),
                ...
                'rolee' => trim(htmlspecialchars($_POST["rolee"])),
        ...
        if ((isset($erreurprod)) && ($erreurprod)) {
            $pag = 'pmfpages/pmfutilisateur/pmfediter/edit_utilisateur';
            $stag = 1;
            die('<meta http-equiv="refresh" content="0; URL='.$url.'?page=' . base64_encode($pag) . '&pagecom=' . $stag . '">');
            
        } else {
        require('pmfpages/pmfutilisateur/pmfediter/form_terminer.php');
        break;
        }
    ...
    default:
        // Valeurs par défaut
        if (empty($_SESSION['formutilisateure'][NUM_PAGE])) {

            $_SESSION['formutilisateure'][NUM_PAGE] = array(
            'idutilisateur' => $_GET["idutilisateur"],               
                'nome' => $row["nomuser"],
                'prenome' => $row["prenomuser"],
                'photostring' => $row["photouser"],
        ...
        } else if (($_SESSION['formutilisateure'][NUM_PAGE]["idutilisateur"]!=$_REQUEST["idutilisateur"])) {
        ...
        require('pmfpages/pmfutilisateur/pmfediter/form_edit_utilisateur.php');
        break;
}
}